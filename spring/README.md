...
AOP解决OOP中遇到一些问题，是OOP的延续和扩展  

##### 底层实现
Spring中的AOP底层实现用到两种代理机制：  
JDK的动态代理		：针对实现了接口的类产生的代理  
Cglib的动态代理	：针对没有实现接口的类产生的代理，应用的是底层的字节码增强技术，生成当前子类对象  
...
* Advice		：通知，指拦截到Joinpoint之后要做的事项，包括前置通知、后置通知、异常通知、最终通知、环绕通知  
* Introduction	：引介，是一种特殊的通知在不修改代码的前提下，Introduction可以在运行期间动态地添加一些方法或Field  
* Target		：代理的目标对象  
* Weaving	：织入，指把增强应用到目标对象来创建新的代理对象的过程  
* spring		：采用动态代理织入，而AspectJ采用编译期织入和类装在期织入    
